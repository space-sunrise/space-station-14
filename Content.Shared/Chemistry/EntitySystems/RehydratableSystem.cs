...
public sealed class RehydratableSystem : EntitySystem
{
    ...
    private void OnSolutionChange(Entity<RehydratableComponent> ent, ref SolutionContainerChangedEvent args)
    {
        ...
        _adminLogger.Add(LogType.Action, LogImpact.Medium, $"{ToPrettyString(ent)} was hydrated, now contains a solution of: {SharedSolutionContainerSystem.ToPrettyString(args.Solution)}.");
    ...
    private void Expand(Entity<RehydratableComponent> ent)
    {
        ...
        _adminLogger.Add(LogType.Action, LogImpact.Medium, $"{ToPrettyString(ent)} has been hydrated correctly and spawned: {ToPrettyString(target)}.");
