...
public partial record struct Marking
{
    ...
    {
        _markingColors = new();
        InitializeSunriseMarkingEffects(); // Sunrise-Edit - синхронизируем эффекты с цветами
    }
    ...
    {
        return GetSunriseHashCode(); // Sunrise-Edit - учитываем эффекты маркировки
    }

    public Marking WithColor(Color color) =>
        WithSunriseColors(Enumerable.Repeat(color, MarkingColors.Count).ToList()); // Sunrise-Edit

    public Marking WithColorAt(int index, Color color)
    {
        var newColors = _markingColors.ShallowClone();
        newColors[index] = color;
        return WithSunriseColors(newColors); // Sunrise-Edit
    }
    ...
    public static Marking? ParseFromDbString(string input) => ParseSunriseDbString(input); // Sunrise-Edit
}
