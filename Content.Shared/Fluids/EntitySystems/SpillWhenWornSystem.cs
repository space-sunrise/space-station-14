...
public sealed class SpillWhenWornSystem : EntitySystem
{
    ...
    {
        if (_solutionContainer.TryGetSolution(ent, ent.Comp.Solution, out var soln, out var solution)
            && solution.Volume > 0)
        {
            // Spill all solution on the player
            var drainedSolution = _solutionContainer.Drain(ent, soln.Value, solution.Volume);
            _puddle.TrySplashSpillAt(ent, Transform(args.Wearer).Coordinates, drainedSolution, out _);
        }
