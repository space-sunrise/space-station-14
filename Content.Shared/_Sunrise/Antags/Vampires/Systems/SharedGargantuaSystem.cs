...
public sealed partial class SharedGargantuaSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<GargantuaComponent, BeforePryEvent>(OnOverwhelmingForceBeforePry);
    ...
    private void OnBloodSwellMeleeDamage(Entity<ActiveBloodSwellComponent> ent, ref GetMeleeDamageEvent args)
    {
        if (!TryComp<StatusEffectComponent>(ent, out var effect)
            ...
            || !TryComp<VampireComponent>(target, out _)
            || !TryComp<VampireProgressionComponent>(target, out var progression))
            return;

        if (args.Weapon != target)
            return;

        if (progression.TotalBlood < ent.Comp.EnhancedThreshold)
            return;
    ...
    private void OnMeleeWeaponDamage(Entity<MeleeWeaponComponent> ent, ref GetMeleeDamageEvent args)
    {
        ...
        if (!_statusEffects.TryEffectsWithComp<ActiveBloodSwellComponent>(args.User, out var effects)
            || !TryComp<VampireComponent>(args.User, out _)
            || !TryComp<VampireProgressionComponent>(args.User, out var userProgression))
        {
            return;
        }
        ...
        {
            if (userProgression.TotalBlood < effect.Comp1.EnhancedThreshold)
                continue;
    ...
    private void OnOverwhelmingForceBeforePry(EntityUid uid, GargantuaComponent component, ref BeforePryEvent args)
    {
        if (!component.OverwhelmingForceActive
            || !TryComp<VampireComponent>(uid, out _)
            || !TryComp<VampireProgressionComponent>(uid, out var pryProgression)
            || pryProgression.TotalBlood >= component.OverwhelmingForceDoorPryBloodCost)
        {
            return;
        }
