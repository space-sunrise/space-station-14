...
ion-storm-law-no-shuttle = ШАТТЛ НЕ МОЖЕТ БЫТЬ ВЫЗВАН ПО ПРИЧИНЕ ПРИСУТСТВИЯ {ION-NUMBER-BASE($ion)} {ION-NUMBER-MOD($ion)} {ION-ADJECTIVE($ion)} {ION-SUBJECT($ion)} НА СТАНЦИИ
...
ion-storm-law-thing-harmful = {ION-THING($ion)} ПРИЧИНЯЮТ ВРЕД ЗДОРОВЬЮ ЭКИПАЖА
