supermatter-examine-integrity = Его целостность составляет {$integrity}%

supermatter-announcement-sender = Автоматическая система двигателя суперматерии

supermatter-announcement-safe =
    Кристаллическая гиперструктура возвращается к безопасным параметрам работы. Аварийная защита отключена.

supermatter-announcement-danger =
    Внимание! Нарушение целостности кристаллической гиперструктуры! Целостность: {$integrity}%

supermatter-announcement-critical =
    Опасность! Целостность кристаллической гиперструктуры достигает критического уровня! Целостность: {$integrity}%

supermatter-announcement-cc-tamper =
    Наша автоматическая система обнаружила, что целостность кристалла суперматерии была нарушена внешним воздействием.
    Инженерный отдел, немедленно отправляйтесь к реактору суперматерии.

supermatter-announcement-delam =
    РАССЛАИВАНИЕ КРИСТАЛЛОВ НЕИЗБЕЖНО. Кристалл суперматерии достиг критического уровня разрушения.

supermatter-announcement-delam-countdown =
    Оценочное время до расслаивания: {$seconds} секунд.

supermatter-announcement-delam-explosion =
    Включение поля дестабилизации для предотвращения потерь.

supermatter-announcement-delam-singuloose =
    Скорое формирование сингулярности.

supermatter-announcement-delam-tesla =
    Скорое высвобождение сжатой энергии.

supermatter-announcement-delam-cascade =
    Превышены пределы гармонической частоты. Поле дестабилизации для предотвращения потерь не удалось активировать.

supermatter-tamper-begin = Вы начинаете аккуратно срезать осколок с кристалла суперматерии...

supermatter-tamper-end = Вы чувствуете себя богом, держа в руках кусок суперматерии. Или это просто радиация?