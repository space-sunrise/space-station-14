...
vending-machine-restock-done-self = Вы завершаете пополнение { THE($target) }.
vending-machine-restock-done-others = { CAPITALIZE(THE($user)) } завершает пополнение { THE($target) }.
