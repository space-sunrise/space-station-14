...
public sealed partial class AntagSelectionSystem : GameRuleSystem<AntagSelectionComponent>
{
    ...
    private void OnPlayerSpawning(RulePlayerSpawningEvent args)
    {
        ...
        var weightedPool = GetSunriseWeightedPlayerPool(pool, _preSpawnRules.Select(rule => rule.Definition)); // Sunrise-Edit
    ...
    private void OnJobsAssigned(RulePlayerJobsAssignedEvent args)
    {
        ...
        // Pick a random player session and then try to assign the currently available antags from it!
        // This means each player has the same chance at rolling antag, with minimal alterations to the odds by number of antags selected.
        var weightedPool = GetSunriseWeightedPlayerPool(args.Players, _postSpawnRules.Select(rule => rule.Definition)); // Sunrise-Edit
        while (RobustRandom.TryPickAndTake(weightedPool, out var session))
        {
    ...
    {
        AssignAntags(gameRule, GetSunriseWeightedPlayerPool(players, antags.Select(antag => antag.Definition)), antags); // Sunrise-Edit
    }
