...
public sealed class SunriseMultiTileAirtightSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        var query = EntityQueryEnumerator<SunriseMultiTileAirtightComponent, TransformComponent>();

        while (query.MoveNext(out var uid, out var multiTile, out var xform))
        {
            ...
            var currentTile = _transform.GetGridTilePositionOrDefault((uid, xform), grid);

            if (multiTile.CachedTile == currentTile && multiTile.CachedGrid == gridUid)
                continue;

            multiTile.CachedTile = currentTile;
            multiTile.CachedGrid = gridUid;

            RefreshGeometry((uid, multiTile));
            RefreshAirblock((uid, multiTile));
        }
    }
