...
public sealed partial class UmbraeSystem : EntitySystem
{
    ...
    private void OnBloodDrank(EntityUid uid, UmbraeComponent umbrae, ref VampireBloodDrankEvent args)
    {
        ...
        if (progression.TotalBlood < umbrae.BreakLightBloodThreshold)
            return;
    ...
    private void OnDarkPassage(EntityUid uid, VampireComponent comp, ref VampireDarkPassageActionEvent args)
    {
        ...
        var isFullPower = TryComp<VampireProgressionComponent>(uid, out var passageProgression) && passageProgression.FullPower;
        if (!_vampire.IsValidTile(target)
            || (!isFullPower
    ...
    private void OnEternalDarkness(EntityUid uid, VampireComponent comp, ref VampireEternalDarknessActionEvent args)
    {
        ...
        {
            if (!TryComp<VampireProgressionComponent>(uid, out var progression) || !progression.FullPower)
            {
    ...
    private bool ConsumeEternalDarknessBlood(EntityUid uid, VampireComponent comp, UmbraeComponent umbrae, int bloodPerTick)
    {
        ...
        if (progression.DrunkBlood < bloodPerTick)
        {
        ...
        float tempDrop)
    {
        ...
        {
            if (ent == uid || !HasComp<HumanoidProfileComponent>(ent) || HasComp<VampireComponent>(ent))
                continue;
    ...
    private void OnShadowAnchor(EntityUid uid, VampireComponent comp, ref VampireShadowAnchorActionEvent args)
    {
        ...
        if (!TryComp<VampireProgressionComponent>(uid, out var shadowAnchorProgression))
            return;

        if (shadowAnchorProgression.TotalBlood < vac.BloodToUnlock)
            return;

        var bloodCost = (int)vac.BloodCost;
        if (bloodCost > 0 && shadowAnchorProgression.DrunkBlood < bloodCost)
        {
    ...
    private void OnShadowBoxingStartAttempt(ref VampireShadowBoxingStartAttemptEvent ev)
    {
        ...
        if (!HasComp<BibleUserComponent>(target)
            || TryComp<VampireComponent>(uid, out _) && TryComp<VampireProgressionComponent>(uid, out var vampProgression) && vampProgression.FullPower
