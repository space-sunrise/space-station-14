...
public sealed partial class DantalionSystem : EntitySystem
{
    ...
    private void OnBloodDrank(EntityUid uid, DantalionComponent dantalion, ref VampireBloodDrankEvent args)
    {
        if (!TryComp<VampireComponent>(uid, out _)
            ...
            || progression.TotalBlood < dantalion.HealBloodThreshold)
            return;
    ...
    /// <summary>
    /// Проверяет, можно ли поработить цель, и запускает doafter
    /// </summary>
    private void OnEnthrall(EntityUid uid, DantalionComponent dantalion, ref VampireEnthrallActionEvent args)
    {
        ...
        if (HasComp<BibleUserComponent>(target) && progression.FullPower != true)
        {
        ...
        if (!HasThrallCapacity(vampire, dantalion, progression))
        {
    ...
    /// <summary>
    /// Повторно проверяет возможность порабощения, списывает кровь вампира, выдаёт цели компонент тхралла, цель, mind-компонент и улей разума, и показывает игроку всплывающее окно
    /// </summary>
    ...
    /// <summary>
    /// Пытается назначить цель тхралла и показывает всплывающее окно при успехе
    /// </summary>
    private void TryAssignThrallObeyObjective(EntityUid master, EntityUid thrall, VampireThrallComponent thrallComp)
    {
        ...
        // показывает pop-up цели о том, что она была порабощена
    ...
    /// <summary>
	///     Вызывается при освобождении тхралла от хозяина. Убирает у игрока антаг-компонент, цели, роль и улей разума
	/// </summary>
    ...
    /// <summary>
	///     Проверяет, хватает ли вампиру крови для выполняемого действия
	/// </summary>
    ...
    private bool HasThrallCapacity(VampireComponent comp, DantalionComponent dantalion, VampireProgressionComponent progression)
        => dantalion.ThrallSlotsUsed < GetThrallLimit(progression, dantalion);

    private int GetThrallLimit(VampireProgressionComponent progression, DantalionComponent dantalion)
    {
        var limit = dantalion.BaseThrallLimit;

        if (progression.TotalBlood >= dantalion.ThrallLevel2Blood)
            limit++;

        if (progression.TotalBlood >= dantalion.ThrallLevel3Blood)
            limit++;
    ...
    private void OnDecoyActivated(ref VampireDecoyActivatedEvent ev)
    {
        ...
        var decoy = Spawn("VampireDecoyEntity", spawnCoords);
    ...
    private bool ProcessBloodBondTick(EntityUid uid, ActiveVampireBloodBondComponent active, DantalionComponent dantalion)
    {
        ...
        if (TryComp<VampireComponent>(uid, out var comp)
            && TryComp<VampireProgressionComponent>(uid, out var bloodBondProgression)
            && bloodBondProgression.DrunkBlood < active.BloodCostPerTick)
        {
            DeactivateBloodBond(uid, dantalion);
        ...
        foreach (var thrall in IterateAndCheckThralls((uid, dantalion)))
        {
            ...
            // Предотвращаем действие лучей связи сквозь стены
    ...
    private void OnMassHysteria(EntityUid uid, DantalionComponent dantalion, ref VampireMassHysteriaActionEvent args)
    {
        ...
        if (!TryComp<VampireComponent>(uid, out var vampire)
            || !TryComp<VampireProgressionComponent>(uid, out var hysteriaProgression))
            return;

        if (!hysteriaProgression.FullPower)
        {
