using Content.Server._Sunrise.Advertise.Components;
...
namespace Content.Server._Sunrise.Advertise.EntitySystems;
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var advert))
        {
            if (currentGameTime > advert.NextAdvertisementTime)
            {
                ...
                // The timer is always refreshed when it expires, to prevent mass advertising (ex: all entities getting power at once).
