...
Этот навык описывает стандарты и паттерны, принятые в форках Space Station 14 с project-folder изоляцией (`_Sunrise`, `_Scp`, `_Fish`, `_Lust`) для работы с кодом, унаследованным от апстрима.
...
Перед внесением изменений сначала определи активный префикс кодбазы, project folder и edit-маркеры по правилу `ss14-codebase-prefix-detection`.
