...
Использование:
    ...
    python3 Tools/_sunrise/find_unused_textures.py --output unused_textures.json
"""
...
def normalize_path(path: str) -> str:
    ...
    # Преобразуем обратные слэши в прямые (для Windows совместимости)
    path = path.replace('\\', '/')
...
def is_texture_referenced(texture_path: str, references: Set[str]) -> bool:
    ...
    # Нормализуем путь
    normalized = normalize_path(texture_path).lower()

    # Проверяем все ссылки
    for ref in references:
        ref_normalized = normalize_path(ref).lower()

        # Прямое совпадение
        if normalized == ref_normalized:
            return True

        # Для RSI директорий проверяем, заканчивается ли ссылка на наш путь
        # Например: "some/path/to/sprite.rsi" может быть ссылкой на "sprite.rsi"
        if normalized.endswith('.rsi'):
            # Проверяем совпадение по имени RSI
            normalized_parts = normalized.split('/')
            ref_parts = ref_normalized.split('/')

            # Если ссылка короче пути, проверяем совпадение суффикса
            if len(ref_parts) <= len(normalized_parts):
                if normalized_parts[-len(ref_parts):] == ref_parts:
                    return True

        # Обратное: ссылка может быть полным путём к файлу в RSI
        if ref_normalized.endswith('.png'):
            # Например: ссылка "Structures/Doors/door.rsi/door.png" на путь "Structures/Doors/door.rsi"
            if ref_normalized.startswith(normalized + '/'):
                return True
...
def main():
    ...
    all_texture_files = get_all_texture_files()
    print(f"Найдено {len(all_texture_files)} текстурных групп")

    if args.verbose:
        print("\nПримеры найденных ссылок:")
        for ref in list(all_refs)[:10]:
            print(f"  - {ref}")
        print(f"\nПримеры текстурных путей:")
        for path in list(all_texture_files.keys())[:10]:
            print(f"  - {path}")
    ...
    for texture_path, files in all_texture_files.items():
    ...
    print("="*80)
    print(f"Всего текстурных групп: {len(all_texture_files)}")
    ...
    output_data = {
            'summary': {
                'total_texture_groups': len(all_texture_files),
