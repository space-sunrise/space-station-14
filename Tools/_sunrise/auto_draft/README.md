...
- Если CodeRabbit явно сообщил о лимите для текущего коммита, ожидание его ревью
...
Учитывается только служебный блок `rate limited by coderabbit.ai` в сообщении
настоящего бота `coderabbitai[bot]`, где указан текущий коммит. Старое уведомление,
обычный текст про лимит, пустой результат или недоступность API не считаются
разрешением. Если CodeRabbit изменит формат уведомления и перестанет указывать
коммит, потребуется обновить распознавание; остаётся ручной аварийный переход.
...
Состояние проверок читается отдельным штатным токеном Actions с правами чтения;
дополнительные разрешения приложению не требуются.
