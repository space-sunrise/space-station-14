...
function buildChecklist({ owner, repo, number, feedback, readiness, manualDraft, manualOverride }) {
  ...
  const lines = [marker, '### Готовим изменения к ревью', '',
    ...
    checkbox(readiness.checksReady, `Пройти обязательные проверки во вкладке [Checks — проверки](${prUrl}/checks). Жёлтая проверка ещё выполняется; красная завершилась с ошибкой.`),
    ...(readiness.checkItems || []).map(item => `  ${checkbox(item.done, plain(item.name))}`),
    '', 'Как найти ошибки тестов:', '',
    '1. Нажми на упавшую проверку во вкладке Checks.',
