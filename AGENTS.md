...
Центр правды для skills и rules - `.agents/skills` и `.agents/rules`.
Папка `.agents/rules` - ЧИТАТЬ ВСЕГДА. Применять всегда, все что написано использовать 100% раз.
Папки `.agent`, `.claude`, `.cursor`, `.github` содержат bridge-файлы и должны ссылаться на `.agents`.
