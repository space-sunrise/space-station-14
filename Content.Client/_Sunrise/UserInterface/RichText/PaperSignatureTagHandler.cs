...
public sealed class PaperSignatureTagHandler : IMarkupTagHandler
{
    public static float FontLineHeight { get; set; }

    public string Name => PaperInteractiveTagParsing.SignatureTagName;
    ...
    public bool TryCreateControl(MarkupNode node, [NotNullWhen(true)] out Control? control)
    {
        ...
        var label = node.Value.TryGetString(out var value)
            ? value
            : Loc.GetString("paper-signature-sign-button");
        ...
        if (FontLineHeight > 0f)
            button.MinHeight = FontLineHeight + 4f;
