...
public sealed partial class SunriseCriminalRecordsCaseEditor : Control
{
    ...
    public SunriseCriminalRecordsCaseEditor()
    {
        ...
        NotesInput.OnTextEntered += _ => Save();
    }

    private long? _lastCaseId;
    ...
    public void Setup(CriminalCase @case, List<CriminalCase> allCases)
    {
        ...
        if (!isSameCase || NotesInput.Text != (@case.Notes ?? ""))
            NotesInput.Text = @case.Notes ?? "";
    ...
    private void Save()
    {
        ...
        OnSave?.Invoke((uint) _lastCaseId.Value, new List<string>(_currentLaws), new List<string>(_currentCircumstances), NotesInput.Text);
    }
    ...
    private void PopulateEditable(CorporateLawsetPrototype lawset)
    {
        ...
        foreach (var entryId in lawset.Circumstances)
        {
            ...
            // Filter out provisions (headers) from selectable list
    ...
    private void UpdateSentenceDisplay()
    {
        ...
        foreach (var lawId in _currentLaws)
        {
        ...
        else
        {
            ...
            var pastLaws = _allCases
                .Where(c => c.Id != _lastCaseId)
                .SelectMany(c => c.Laws)
                .ToHashSet();

            foreach (var lawId in _currentLaws.Distinct())
            {
