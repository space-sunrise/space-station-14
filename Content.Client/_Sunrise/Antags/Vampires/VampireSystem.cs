...
public sealed partial class VampireSystem : EntitySystem
{
    ...
    private void OnUpdateAlert(EntityUid uid, VampireComponent comp, ref UpdateAlertSpriteEvent args)
    {
        ...
        if (key == "VampireBlood")
        {
            ...
            // Фон задаётся алертом -> выставляем только слои цифр из значения счётчика.
            var value = Math.Clamp(progression.DrunkBlood, 0, 9999);
            var d1 = value / 1000 % 10;
            var d2 = value / 100 % 10;
            var d3 = value / 10 % 10;
            var d4 = value % 10;

            _sprite.LayerSetRsiState((args.SpriteViewEnt, args.SpriteViewEnt.Comp), VampireBloodVisualLayers.Digit1, d1.ToString());
            _sprite.LayerSetRsiState((args.SpriteViewEnt, args.SpriteViewEnt.Comp), VampireBloodVisualLayers.Digit2, d2.ToString());
            _sprite.LayerSetRsiState((args.SpriteViewEnt, args.SpriteViewEnt.Comp), VampireBloodVisualLayers.Digit3, d3.ToString());
            _sprite.LayerSetRsiState((args.SpriteViewEnt, args.SpriteViewEnt.Comp), VampireBloodVisualLayers.Digit4, d4.ToString());
        }
    }
    ...
    {
        if (bloodCost <= 0 || !TryComp<VampireProgressionComponent>(uid, out var progression) || progression.DrunkBlood >= bloodCost)
            return true;
