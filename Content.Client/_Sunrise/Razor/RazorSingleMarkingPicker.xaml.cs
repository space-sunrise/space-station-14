...
public sealed partial class RazorSingleMarkingPicker : BoxContainer
{
    ...
    private HumanoidVisualLayers _category;
    public HumanoidVisualLayers Category
    {
        ...
        {
            _category = value;
            CategoryName.Text = Loc.GetString($"markings-layer-{_category}");
    ...
    public void UpdateData(List<Marking> markings, string species, Sex sex, int totalPoints)
    {
        ...
        _markingPrototypeCache = MarkingsByLayerAndSpecies(Category, _species!);
    ...
    private void SelectMarking(ItemList.ItemListSelectedEventArgs args)
    {
        ...
        if (!_prototype.TryIndex<MarkingPrototype>(id, out var proto))
        {
    ...
    private IReadOnlyDictionary<string, MarkingPrototype> MarkingsByLayerAndSpecies(
        HumanoidVisualLayers layer,
        ProtoId<SpeciesPrototype> species)
    {
        var markingData = _markingManager.GetMarkingData(species);
        foreach (var data in markingData.Values)
        {
            if (!data.Layers.Contains(layer))
                continue;

            return _markingManager.MarkingsByLayerAndGroupAndSex(layer, data.Group, _sex);
        }
