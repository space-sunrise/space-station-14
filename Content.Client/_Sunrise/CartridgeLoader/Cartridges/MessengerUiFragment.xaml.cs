...
public sealed partial class MessengerUiFragment : BoxContainer
{
    ...
    private DefaultWindow? _createGroupDialog;
    private DefaultWindow? _addUserDialog;
    ...
    private void UpdateMessages(List<MessengerMessage> messages)
    {
        ...
        for (int i = _lastMessageCount; i < messages.Count; i++)
        {
            ...
            messagePanel.PanelOverride = roundedStyleBox;
            ...
            rightContainer.AddChild(timeLabel);

            header.AddChild(rightContainer);
            ...
            if (isPersonalChat && isOwnMessage && message.RecipientId != null)
            {
                ...
                var readStatusIcon = new TextureRect
                {
                    Texture = _resourceCache.GetTexture(readStatusTexturePath),
                    SetWidth = 16,
                    SetHeight = 16,
                ...
                contentContainer.AddChild(readStatusIcon);

                messageBox.AddChild(header);
                messageBox.AddChild(contentContainer);
            }
            else
            {
                var contentLabel = new RichTextLabel
                ...
                messageBox.AddChild(header);
                messageBox.AddChild(contentLabel);
            }

            messagePanel.AddChild(messageBox);
    ...
    private void ShowCreateGroupDialog()
    {
        ...
        {
            Title = Loc.GetString("messenger-create-group-title"),
        ...
        var nameLabel = new Label
        {
            Text = Loc.GetString("messenger-create-group-label"),
            Margin = new Thickness(0, 0, 0, 4)
        };
        ...
        {
            PlaceHolder = Loc.GetString("messenger-create-group-placeholder"),
            HorizontalExpand = true,
        ...
        {
            Text = Loc.GetString("messenger-create-group-button-create"),
            HorizontalExpand = true,
        ...
        {
            Text = Loc.GetString("messenger-create-group-button-cancel"),
            HorizontalExpand = true
        };
        ...
        cancelButton.OnPressed += _ => dialog.Close();
    ...
    private void ShowAddUserToGroupDialog(string groupId, string groupName)
    {
        ...
        if (availableUsers.Count == 0)
        {
            ...
            noUsersDialog.OnClose += () => _addUserDialog = null;
        ...
        var dialog = new DefaultWindow
        {
            Title = Loc.GetString("messenger-add-user-to-group-title", ("groupName", groupName)),
            MinSize = new Vector2(300, 400)
        };
        _addUserDialog = dialog;

        var dialogContainer = new BoxContainer
        {
            Orientation = LayoutOrientation.Vertical,
            HorizontalExpand = true,
            VerticalExpand = true,
        ...
        {
            Text = Loc.GetString("messenger-add-user-search"),
            Margin = new Thickness(0, 0, 0, 4)
        };
        dialogContainer.AddChild(searchLabel);

        var searchInput = new LineEdit
        {
            PlaceHolder = Loc.GetString("messenger-add-user-placeholder"),
            HorizontalExpand = true,
            Margin = new Thickness(0, 0, 0, 8)
        };
        dialogContainer.AddChild(searchInput);
        ...
        var usersList = new BoxContainer
        {
            Orientation = LayoutOrientation.Vertical,
            HorizontalExpand = true
        };
        ...
        scrollContainer.AddChild(usersList);
        dialogContainer.AddChild(scrollContainer);

        var closeButton2 = new Button
        {
            Text = Loc.GetString("messenger-add-user-cancel"),
            HorizontalExpand = true,
            Margin = new Thickness(0, 4, 0, 0)
        };
        closeButton2.OnPressed += _ => dialog.Close();
        dialogContainer.AddChild(closeButton2);

        dialog.Contents.AddChild(dialogContainer);
